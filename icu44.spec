...
Source1:   icu-config
BuildRoot: %{_tmppath}/icu-%{version}-%{release}-root-%(%{__id_u} -n)
...
Patch9: icu-4.4-with-version.patch
...
%configure --with-data-packaging=library \
           --disable-samples \
           --includedir=%{_includedir}/%{name} \
           --with-library-suffix=44
#rhbz#225896
...
sed -i s/\\\$\(THREADSCXXFLAGS\)// $RPM_BUILD_ROOT/%{_libdir}/pkgconfig/%{name}.pc
sed -i s/\\\$\(THREADSCPPFLAGS\)/-D_REENTRANT/ $RPM_BUILD_ROOT/%{_libdir}/pkgconfig/%{name}.pc
for binary in %{buildroot}%{_bindir}/* %{buildroot}%{_sbindir}/*; do
  if [ "`basename ${binary}`" != "%{name}-config" ]; then 
    mv ${binary} ${binary}%{pversion}
  fi
done
for manpage in %{buildroot}%{_mandir}/man1/*; do
  trunk=`basename ${manpage} .1`
  if [ "${trunk}" == "icu-config" ]; then 
    mv ${manpage} %{buildroot}%{_mandir}/man1/%{name}-config.1
  else
    mv ${manpage} %{buildroot}%{_mandir}/man1/${trunk}%{pversion}.1
  fi
done
for manpage in %{buildroot}%{_mandir}/man8/*; do
  trunk=`basename ${manpage} .8`
  mv ${manpage} %{buildroot}%{_mandir}/man8/${trunk}%{pversion}.8
done
...
%changelog
* Thu Oct 13 2011 Jean-Paul Chaput <Jean-Paul.Chaput@lip6.fr>
- Imported from Fedora 15 to SL6 as dependency for Chromium 14.
- Renamed into icu44 to install in parallel to the system package.
